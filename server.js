...
app.get('/country', async function (req, res) {
  ...
  if (result.rowCount < 1) {
    client.release();
    return res.status(500).send("Internal Error - No Country Found");
  }
...
app.get('/airlines/search', async function (req, res) {
  ...
  const result1 = await client.query(
    "SELECT name, iata, icao FROM airlines WHERE (iata = $1 OR $1 IS NULL) AND (icao = $2 OR $2 IS NULL)",
    [iata, icao]
  );
  if (result1.rowCount < 1) {
    client.release();
    return res.status(500).send("Internal Error - No airlines Found");
  }
  res.set("Content-Type", "application/json");
  res.send(result1.rows);
  client.release();
...
app.post('/airlines', async function (req, res) {
  ...
  const client = await pool.connect();
  await client.query(
    "INSERT INTO airlines (name, icao, iata, country) VALUES ($1, $2, $3, $4) RETURNING *",
    [name, icao || null, iata || null, country]
  );
...
app.delete('/airlines', async function (req, res) {
  ...
  const client = await pool.connect();
  const result1 = await client.query(
    "DELETE FROM airlines WHERE iata = $1::text OR icao = $2::text", [iata, icao]
  );
...
app.get('/airports', async function (req, res) {
  ...
  const client = await pool.connect();
  const result1 = await client.query(
    "SELECT name, iata, icao FROM airports WHERE country = $1::text", [country]
  );
  if (result1.rowCount < 1) {
    client.release();
    return res.status(500).send("Internal Error - No Country Found");
...
app.get('/airports/search', async function (req, res) {
  ...
  const client = await pool.connect();
  const result1 = await client.query(
    "SELECT name FROM airports WHERE iata = $1::text OR icao = $2::text", [iata, icao]
  );
  if (result1.rowCount < 1) {
    client.release();
    return res.status(500).send("Internal Error - No airports Found");
  }
  res.set("Content-Type", "application/json");
  res.send(result1.rows);
  client.release();
...
app.post('/airports', async function (req, res) {
  ...
  const client = await pool.connect();
  await client.query(
    "INSERT INTO airports (name, icao, iata, country) VALUES ($1, $2, $3, $4) RETURNING *",
    [name, icao || null, iata || null, country]
  );
...
app.delete('/airports', async function (req, res) {
  ...
  const client = await pool.connect();
  const result1 = await client.query(
    "DELETE FROM airports WHERE iata = $1::text OR icao = $2::text", [iata, icao]
  );
  if (result1.rowCount > 0) {
    res.status(200).send("Airport deleted successfully.");
  } else {
...
app.get('/routes', async (req, res) => {
  ...
  const client = await pool.connect();
  const result1 = await client.query(
    `SELECT a1.iata AS dep_iata, a1.latitude AS dep_lat, a1.longitude AS dep_lon,
            a2.iata AS arr_iata, a2.latitude AS arr_lat, a2.longitude AS arr_lon,
            r.airline, r.planes
     FROM airports a1
     JOIN airports a2 ON a1.iata = $1 OR a2.iata = $2
     LEFT JOIN routes r ON r.depature = a1.iata AND r.arrival = a2.iata
     WHERE (a1.iata = $1 OR a2.iata = $2)
       AND r.airline IS NOT NULL
       AND r.planes IS NOT NULL`,
    [dep, arr]
  );
...
app.get('/routes/arrival-airport', async (req, res) => {
  ...
  const client = await pool.connect();
  const result = await client.query(
    `SELECT a2.iata AS arrival_iata, a2.name AS arrival_name, a2.city AS arrival_city, a2.country AS arrival_country
     FROM routes r
...
app.get('/routes/airline-planes', async (req, res) => {
  ...
  const client = await pool.connect();
  const result = await client.query(
    `SELECT r.depature AS departure, r.arrival AS arrival,
            a1.name AS departure_name, a2.name AS arrival_name
     FROM routes r
     JOIN airports a1 ON r.depature = a1.iata
     JOIN airports a2 ON r.arrival = a2.iata
     WHERE r.airline = $1 AND r.planes = $2`,
    [airline, plane]
  );
  if (result.rows.length === 0) {
    client.release();
...
app.get('/routes/arriving-at-airport', async (req, res) => {
  ...
  const result = await client.query(
    `SELECT r.depature AS departure, r.arrival AS arrival,
            a1.name AS departure_name, a2.name AS arrival_name, r.airline
     FROM routes r
     JOIN airports a1 ON r.depature = a1.iata
     JOIN airports a2 ON r.arrival = a2.iata
     WHERE r.arrival = $1`,
...
app.delete('/routes', async (req, res) => {
  ...
  const client = await pool.connect();
  const result = await client.query(
    `DELETE FROM routes WHERE airline = $1 AND depature = $2 AND arrival = $3 RETURNING *`,
...
app.put('/routes/update', async (req, res) => {
  ...
  const newAircraftTypesToAdd = planes.filter((type) => !existingPlanes.includes(type));
  if (newAircraftTypesToAdd.length === 0) {
    client.release();
    return res.status(400).send('No new aircraft types to add.');
  }
...
app.get('/airports/temp', async (req, res) => {
  ...
  res.json({
    ...
    high: dailyWeather.temperature_2m_max[0],
    low: dailyWeather.temperature_2m_min[0],
  });
  client.release();
});
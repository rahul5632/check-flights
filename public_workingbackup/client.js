...
(function () {
  ...
  async function fetchAirportData(iataOrIcao) {
    ...
    if (airportInfo.ok && departureRoutes.ok && arrivalRoutes.ok) {
      ...
      const airlineDetails = await fetchAirlineNames(Array.from(airlinesSet).filter(a => a));
      ...
      document.getElementById("airport-results").innerHTML = `
        ...
        <ul>
          ${departures.map(r => `<li>${info.name} (${info.iata || info.icao}) ➔ ${r.arrival_name} (${r.arrival_iata})</li>`).join('')}
        </ul>

        <h3>Routes Arriving at this Airport:</h3>
        <ul>
          ${arrivals.map(r => `<li>${r.departure_name} (${r.departure}) ➔ ${info.name} (${info.iata || info.icao})</li>`).join('')}
        </ul>

        <h3>Airlines Flying To/From this Airport:</h3>
        <ul>
          ${airlineDetails.length > 0 ? airlineDetails.map(a => `<li>${a.name} (${a.iata || a.icao})</li>`).join('') : "<li>No airlines found</li>"}
        </ul>
      `;
  ...
  async function fetchAirlineNames(airlineCodes) {
    const airlineDetails = [];

    for (const code of airlineCodes) {
      const params = new URLSearchParams({ iata: code });
      const res = await fetch(`/airlines/search?${params}`);
      if (res.ok) {
        const data = await res.json();
        if (data.length > 0) {
          airlineDetails.push(data[0]);
        }
      }
    }
